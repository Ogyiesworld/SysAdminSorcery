...
Write-Output '--------------'
Write-Output "Username: $FirstLetter$Surname"
Write-Output '--------------'
Write-Output "User Principal Name: $FirstLetter$Surname@$localDomain"
Write-Output '--------------'
Write-Output "Email Address: $FirstLetter$surname@$EmailAddressDomain'"
Write-Output '--------------'
...
Invoke-Command -ComputerName "$DCServerHostname" -ScriptBlock {
    if (Get-ADUser -Filter { SamAccountName -eq "$FirstLetter$Surname" }) {
...
$Params = @{
    Name              = $Name
    SamAccountName    = "$FirstLetter$Surname"
    GivenName         = $FirstName
    Surname           = $Surname
    DisplayName       = "$FirstName $Initial $Surname"
    EmailAddress      = "$FirstLetter$surname@$EmailAddressDomain"
    Country           = "US"
    UserPrincipalName = "$FirstLetter$Surname@$localDomain"
    Path              = $OUPath
...
try {
    ...
    } -ArgumentList "$FirstLetter$Surname"
...
try {
    Get-ADUser -Identity "$FirstLetter$Surname" -Properties *
}
catch {
